...
function plot_gershgorin(A)
    ...
    for disk in disks[1:3]
        center = disk[1]
        radius = disk[2]
        scatter!(ax3, [center 0], color = :blue, markersize = 4, marker = :circle)
        truc = radius .* eit .+ complex(center)
        lines!(ax3, real(truc),imag(truc), color = :blue, linewidth = 2)
    end
    display(f)
    return f
end
...
#K = 100
# ω = 1
spaceF = CosFourier(K, mid(ω))
p = Sequence(spaceF, rand(Float64, K+1).*[ (-1/2)^k for k in 0:K])#Sequence(spaceF, coefficients(mid.(P_bar[1])))#
xx = LinRange(0, π/mid(ω), 2*K+1)
lines(xx, x -> p(x))
Matp = [p zero(p); zero(p) p]
if !isinv(p)
       error("not invertible")
end
spaceBig = CosFourier(10K, mid(ω))
...
eig_fin = eigen(coefficients(P_finite))
Multp = project(Multiplication(p), spaceBig, spaceBig)
eig_Multp = eigen(coefficients(Multp))